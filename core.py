...
def gpt3_completion(prompt, engine='text-davinci-003', temp=0.8, top_p=1.0, n=1, best_of=1, tokens=400, freq_pen=1.1, pres_pen=0.0, stop=['USER:', 'LUMIERE:']):
    max_retry = 5
    retry = 0
...
if __name__ == '__main__':
    convo_length = 30
    [INSERT API KEYS HERE FOR OPENAI AND PINECONE]
    vdb = pinecone.Index("online-lumiere-mvp")
    while True:
        ...
        vector = gpt3_embedding(message)
        unique_id = str(uuid4())
        metadata = {'speaker': 'LUMIERE', 'time': timestamp, 'message': message, 'timestring': timestring, 'uuid': unique_id}
        save_json('nexus/%s.json' % unique_id, metadata)
        payload.append((unique_id, vector))
        vdb.upsert(payload)
        print('\n\nLUMIERE: %s' % output) 